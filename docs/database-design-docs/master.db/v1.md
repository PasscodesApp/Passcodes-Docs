...
!!! WARNING
	...
	previous database schema (v0) was not migrated, but a new database was created from ground up...

	so, you won't find any migration sql here.

- **Used by**: `v1.0.0` => `v1.2.1`
...
!!! DANGER
	This is `destructive` database migration. apply on your responsibility. after proper data backup.

```sql
-- Migration is not avaliable. As Database was recreated, but you still use below sql to migrated.
-- It is destructive migration, It destory history for passwords completely. (old id's, along with createdat & updateat data will lost forever).

/*
BEGIN TRANSACTION;

-- Step 1: Convert NULL notes to empty strings ('')
UPDATE `passwords` SET `notes` = '' WHERE `notes` IS NULL;

-- Step 2: Create new table with v1 schema
...
??? Note "Migration Not Available"
	Revert migration is not avaiable as database (`v1`) does have a inherit parent schema.
	clearly, `v0` is not a parent schema of `v1`.
