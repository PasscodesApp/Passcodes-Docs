# User docs

!!! Warning

    It is just a open source project. An is current under active development.
    Please, consider using it for fun, and not for real password management. (until, we officially
    release a stable release)

Here you will find all the docs for users.

- [Installion Guide](installing.md)
- [Security Guidelines](security-guidelines.md)
- [Release Notes](release-notes.md)
- [Changelog](changelog.md)
- [Visit Website](https://jeeldobariya38.github.io/Passcodes-Website/)

## Screen Shots

![screenshot](https://github.com/user-attachments/assets/b6407ba3-be7f-4a5b-9e17-8e201ecf2ff3){:width="35%"}
![screenshot](https://github.com/user-attachments/assets/930c18aa-7e38-4a56-a8e4-f5f1d2ee6e38){:width="35%"}
![screenshot](https://github.com/user-attachments/assets/135f29cb-97ed-4cc3-acd6-4a606e9d8844){:width="35%"}
![screenshot](https://github.com/user-attachments/assets/0c84450e-608d-4845-8771-2adba562e48d){:width="35%"}

## Compatibility

**Compile Sdk**: `Android 16 (API level 36)`

- **Minimum**: Android 8+ [API level 26+]
- **Maximum**: Android 14 [API level 34] (we support offically utill here)

Note: high version can still run, but are not guaranteed offically.

## Features

You can see all [features over here](features.md).

## Getting Started

1. Go to our [Github Release](https://github.com/PasscodesApp/Passcodes/releases/latest) OR [Website](https://jeeldobariya38.github.io/Passcodes-Website/).

2. Download the apk for your phone. If, don't know the architecture of phone then download apk file
   that has universal in its name.

3. Install the apk and you are ready to use the app.

It was most shortest and sweetest description (I have ever crafted), For more detailed description,
See the file [installing.md](installing.md) in `PasscodesApp/Passcodes-Docs`.

## Changelog & Release Notes

- Professional ChangeLog: [Changelog](changelog.md)
- Casual Release Notes: [Release Notes](release-notes.md)

## Contribution Are Appreciated!!!

!!! Failure

    Currently the project (more or less also me) is not ready for (code) contributions.
    This project is currently limited by my expertise & I am beginner. Meaning I don;t want others
    code in my codebase, that I don't understand, I am learning android and will surely learn basic
    off by start of 2026.. then i will allow the contribution to this repo... But as of now i don't
    admire code contribution. As I am beginner in android and might have difficultly in understanding
    code that not written by me...

    There are still other ways to contribute to project.. like by testing app, by contributing to
    documentation or more or less just by spreading the word around.. about the app. but, code
    contribution is limited from my side.... & I hope you can understand...

    For more, information on what kind of contribution we approve or allow, please open a issue in our
    repo. that way you avoid wasting your time...

    [Read More](../references/open-contributing-timeline.md)

By, contributing to project you accept the [Terms For Contributions](../other-docs/terms-for-contributions.md),
[CONTRIBUTING](../CONTRIBUTING.md) & [MIT License](../LICENSE.md).

And also have look at [Open Contributing Timeline](../references/open-contributing-timeline.md).

## Security Policy

Available here in [PasscodesApp/Passcodes-Docs](../SECURITY.md).

## License

Passcodes Project is license under [MIT](../LICENSE.md) License.
Downloading Or Use the app or any associated stuff would mean, you are okay and have accepted the license terms.
